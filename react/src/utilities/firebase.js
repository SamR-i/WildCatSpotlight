...
export const writeEventData2 = async (event, imageFile, editID) => {
    ...
    if (imageFile) {
        ...
        try {
            ...
            if (editID) {
                const eventDocRef = doc(firestore, "events", editID.toString());
                await setDoc(eventDocRef, event);
                console.log('Event data updated in Firestore');
            } else {
                const eventDocRef = doc(firestore, "events", event.eventID.toString());
                await setDoc(eventDocRef, event);
                console.log('Event data written to Firestore');
            }
        } catch (error) {
            console.error("Error uploading image and writing event data: ", error);
        }
