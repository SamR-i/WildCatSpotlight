import { updateDoc, collection, deleteDoc, doc, getDoc, getDocs, setDoc } from "firebase/firestore";
...
const ProfilePage = ({ userId, setClicked, editID, onEventClick}) => {
  ...
  const deleteEvent = async (eventID) => {
    try {
      // delete event in events firebase
      const eventDocRef = doc(firestore, "events", eventID.toString());
      await deleteDoc(eventDocRef);
      // delete event in your hosting array
      const userDocRef = doc(firestore, 'users', userId);
      console.log(eventID.toString());
      const userDocSnapshot = await getDoc(userDocRef);
      // await updateDoc(userDocRef, 
      //     hostingEvents: firestore.FieldValue.arrayRemove(eventID.toString())
      // });
      // now need to delete it from everyone's attending and bookmarked arrays
      const usersQuerySnapshot = await getDocs(collection(firestore, 'users'));

      usersQuerySnapshot.forEach(async (userDoc) => {
        const userData = userDoc.data();
        const userID = userDoc.id;
        // first delete from all hosting events. Even though just yours, previous code not working.
        if (userData.hostingEvents && userData.hostingEvents.includes(eventID.toString())) {
          const updatedHostingEvents = userData.hostingEvents.filter(id => id !== eventID.toString());

          await updateDoc(doc(firestore, 'users', userID), {
            hostingEvents: updatedHostingEvents
          });
        }
        // first delete it from the attendingEvents array for each user
        if (userData.attendingEvents && userData.attendingEvents.includes(eventID.toString())) {

          const updatedAttendingEvents = userData.attendingEvents.filter(id => id !== eventID.toString());
        ...
        // second delete it from the favorite events array for each user
        if (userData.favoriteEvents && userData.favoriteEvents.includes(eventID.toString())) {

          const updatedFavoriteEvents = userData.favoriteEvents.filter(id => id !== eventID.toString());
      ...
      const updatedEventsData3 = eventsData3.filter(event => event.eventID !== eventID);
      setEventsData3(updatedEventsData3);
      const updatedEventsData2 = eventsData2.filter(event => event.eventID !== eventID);
      setEventsData2(updatedEventsData2);
      const updatedEventsData = eventsData.filter(event => event.eventID !== eventID);
  ...
  useEffect(() => {
    const newBkLength = eventsData.length;
    setBkNumber(newBkLength);
  }, [eventsData]);
  useEffect(() => {
    const newAttendLength = eventsData2.length;
    setAttendNumber(newAttendLength);
  }, [eventsData2]);
  useEffect(() => {
    const newHostingLength = eventsData3.length;
    setHostNumber(newHostingLength);
  }, [eventsData3]);
  // setBkNumber(eventsData3.length);
  // setAttendNumber(aCount);
  // setHostNumber(hCount);
                ...
                <div className="events-container">
                  ...
                  <ul>
                    {eventsData3.map((event, index) => (
                      <li key={index} className="event-item" onClick={()=>onEventClick(event)}>
                        ...
                        <div className="event-details">
                          ...
                          <div className="two-buttons">
                            <button className="delete-button" onClick={() => deleteEvent(event.eventID)}>
                              <FaTrashAlt />
                            </button>
                            <button className="edit-button" onClick={() => setClickto6(event.eventID)}>
                              <BiEditAlt />
                            </button>
                          </div>
                        </div>
