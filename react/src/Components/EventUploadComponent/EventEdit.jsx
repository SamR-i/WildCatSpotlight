...
import { addEventToOrganizer, auth, getUserInfo, writeEventData2, writeEventData } from "../../utilities/firebase";
import CalendarInput from './CalendarInput';
import LocationInput from './LocationInput';
import { getEventData } from '../../utilities/firebase';
// import { firestore } from '../../utilities/firebase';

const EventEdit = ({ onCancelButtonClick, onPostButtonClick, updateEventData, editID, setEditID }) => {
    ...
    useEffect(() => {
        if (editID) {
            // Fetch event data using editID
            const fetchData = async () => {
                const eventData = await getEventData(editID);
                if (eventData) {
                    // Set state with fetched event data
                    setName(eventData.eventName || '');
                    setLocation(eventData.eventLoc || '');
                    setLatitude(eventData.latitude || '');
                    setLongitude(eventData.longitude || '');
                    setDescription(eventData.eventDesc || '');
                    setStartDate(eventData.eventStart || '');
                    setEndDate(eventData.eventEnd || '');
                    setMaxCap(eventData.eventCapacity || '');
                    setTicketlink(eventData.eventTicket || '');
                    setSelectedCategory(eventData.eventType || '');
                    setDuration(eventData.eventDuration || 2);
                    setDurationUnit(eventData.eventDurationUnit || 'hour');
                    setOrgName(eventData.orgName || '');
                    setUserID(eventData.userID || '');
                    setImagePreview(eventData.eventPhoto || '');
                }
            };
            fetchData();
        }
    ...
    function makeJSONFirebase(editId) {
    ...
    const handleConfirmation = async (confirmed) => {
        if (confirmed) {
            ...
            const data = makeJSONFirebase(editID);
    
            if (editID) {
                await writeEventData2(data, imageFile, editID); 
            } else {
                await writeEventData(data, imageFile);
                await addEventToOrganizer(data["userID"], data["eventID"]);
            }
            clearField();
            onPostButtonClick();
            updateEventData();
    ...
    return (
        <div className='main-container'>
            <p className='page-title'>Edit Your Event</p>
                ...
                {showConfirmation && (
                    <div className="confirmation-modal">
                        <div className="confirmation-content">
                            <p>Are you sure these are the changes you want to make?</p>
